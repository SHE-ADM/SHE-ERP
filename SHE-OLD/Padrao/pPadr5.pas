...
procedure TFrmPadr5._SetCurrentEvent(const AValue: String);
begin
  FCurrentEvent := AValue;
end;

procedure TFrmPadr5._SetCurrentAlert(const AValue: String);
begin
  FCurrentAlert := AValue;
end;

procedure TFrmPadr5._SetForceClose(const AValue: Boolean);
begin
  FForceClose := AValue;
end;

procedure TFrmPadr5._WM_CREATE(var Msg: TMessage);
begin
  { INICIALIZA }
...
procedure TFrmPadr5._WM_ACTIVATE(var Msg: TMessage);
begin
  { EVENTOS }
  ACTEveRegister.Execute; { Registro }
end;

procedure TFrmPadr5._SW_SHOWNOACTIVATE(var Msg: TMessage);
...
procedure TFrmPadr5._WM_SHOW(var Msg: TMessage);
begin
  { BEFORE SHOWNING }
...
procedure TFrmPadr5._WM_AFTER_RESIZE(var Message: TMessage);
begin
  { After Resize }
...
Constructor TFrmPadr5.Create(AOwner: TComponent;
                         const AIDPK : LongInt = 0 ;
                               ADEPK : String  = '';
                               AIDEV : LongInt = 0 ;
                               ACDEV : Word    = 0 ;
...
Class procedure TFrmPadr5._ExecForm(AOwner : TComponent;var AForm; AFormPesquisa: Boolean = False; AFormStyle: TFormStyle = fsMDIChild;
                                      AIDPK : LongInt = 0 ;
                                      ADEPK : String  = '';
                                      AIDEV : LongInt = 0 ;
                                      ACDEV : Word    = 0 ;
...
procedure TFrmPadr5.FormCreate(Sender: TObject);
begin
  { VALIDATE GRANT USER }
...
procedure TFrmPadr5.FormShow(Sender: TObject);
begin
  OnShow := Nil;
  if FForceClose then
  Exit;
...
procedure TFrmPadr5.FormActivate(Sender: TObject);
begin
  OnActivate := Nil;
  if FForceClose then
  Exit;
...
procedure TFrmPadr5.FormCloseQuery(Sender: TObject;
...
procedure TFrmPadr5.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  OnClose := Nil;
  Action  := caFree;
end;

procedure TFrmPadr5.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
...
procedure TFrmPadr5.FormPaint(Sender: TObject);
var
...
procedure TFrmPadr5.FormResize(Sender: TObject);
begin
  if (not Showing) or (FForceClose) then
  Exit;
...
procedure TFrmPadr5.ACTRefreshExecute(Sender: TObject);
begin
  oRefresh(Consulta);
end;
...
procedure TFrmPadr5.ACTMPEditExecute(Sender: TObject);
begin
  { nothing }
end;

procedure TFrmPadr5.ACTMPDeleteExecute(Sender: TObject);
begin
  { nothing }
end;
...
procedure TFrmPadr5.ACTDashboardsExecute(Sender: TObject);
begin
  { nothing }
end;

procedure TFrmPadr5.ACTProgressBarExecute(Sender: TObject);
begin
  { nothing }
end;

procedure TFrmPadr5.ACTEveRegisterExecute(Sender: TObject);
begin
  { UNREGISTER EVENTS }
...
procedure TFrmPadr5.ACTEveExpressExecute(Sender: TObject);
begin
  ACTEveRegister.Execute;
  ACTEveExecute.Execute;
end;

procedure TFrmPadr5.EEventEventAlert(Sender: TObject; EventName: String;
  EventCount: Integer; var CancelAlerts: Boolean);
begin
...
procedure TFrmPadr5.ConsultaBeforeOpen(DataSet: TDataSet);
begin
  { INICIALIZA FORM SCREEN }
  Screen.Cursor := crHourGlass;
end;

procedure TFrmPadr5.ConsultaAfterOpen(DataSet: TDataSet);
begin
...
procedure TFrmPadr5.ConsultaBeforeClose(DataSet: TDataSet);
begin
  { nothing }
end;

procedure TFrmPadr5.ConsultaAfterClose(DataSet: TDataSet);
begin
  SBRodape.Panels[1].Text := EmptyStr;
end;

procedure TFrmPadr5.ConsultaBeforeInsert(DataSet: TDataSet);
begin
...
procedure TFrmPadr5.ConsultaNewRecord(DataSet: TDataSet);
begin
  { nothing }
end;
...
procedure TFrmPadr5.ConsultaBeforeDelete(DataSet: TDataSet);
begin
...
procedure TFrmPadr5.ConsultaBeforePost(DataSet: TDataSet);
begin
  { nothing }
end;
...
procedure TFrmPadr5.ConsultaBeforeCancel(DataSet: TDataSet);
begin
...
procedure TFrmPadr5.ConsultaAfterCancel(DataSet: TDataSet);
begin
  { nothing }
end;

procedure TFrmPadr5.ConsultaUpdateError(DataSet: TDataSet;
...
procedure TFrmPadr5.ConsultaEditError(DataSet: TDataSet; E: EDatabaseError;
  var Action: TDataAction);
begin
  DataBaseError(oFBException(E.Message));
end;

procedure TFrmPadr5.ConsultaDeleteError(DataSet: TDataSet;
  E: EDatabaseError; var Action: TDataAction);
begin
  DataBaseError(oFBException(E.Message));
end;

procedure TFrmPadr5.ConsultaPostError(DataSet: TDataSet; E: EDatabaseError;
  var Action: TDataAction);
begin
  DataBaseError(oFBException(E.Message));
end;

procedure TFrmPadr5.ConsultaCalcFields(DataSet: TDataSet);
begin
...
procedure TFrmPadr5.DBGConsultaBackgroundDrawEvent(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect);
var
  AValueText: String;
begin
...
procedure TFrmPadr5.DBGConsultaCustomDrawCell(Sender: TObject;
...
procedure TFrmPadr5.DBGConsultaDblClick(Sender: TObject);
begin
  if ACTMEEdit.Enabled then ACTMEEdit.Execute else
  if ACTMPEdit.Enabled then ACTMPEdit.Execute;
end;

procedure TFrmPadr5.DBGConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  case key of
