...
uses uPrincipal, bPrincipal,
  pemail, pnfe_cce, pPesquisa_OLD, pven_nfe,
  pNFeConsultaSefaz;
...
begin
  inherited;
  try
    frmnfe_cce := Tfrmnfe_cce.Create(Nil);
...
begin
  if Length(CadastroNFE_CHAV.AsString) = 44 then
     try
       ...
       try
         FrmNFeConsultaSefaz := TFrmNFeConsultaSefaz.Create(Nil,'Consultando NFe ...',CadastroNFE_CHAV.AsString);
...
begin
  ...
  begin
    FrmNFeConsultaSefaz := TFrmNFeConsultaSefaz.Create(Nil,'Cancelando NFe ...',CadastroNFE_CHAV.AsString);
    try
  ...
  try
    ...
    begin
      ...
      begin
        ...
        SQL.Add(IFThen(Pos('WHERE',Cadastro.Text) = 0,'WHERE ','AND ') + FrmPesquisa_OLD.cData + ' BETWEEN ''' + FormatDateTime('mm/dd/yy',FrmPesquisa_OLD.dxDT1.Date) + ''' AND ''' + FormatDateTime('mm/dd/yy',FrmPesquisa_OLD.dxDT2.Date) + '''');
    ...
    begin
      APSQ_FIS_NFE := FrmPesquisa_OLD.cbCAMPO.Text;
