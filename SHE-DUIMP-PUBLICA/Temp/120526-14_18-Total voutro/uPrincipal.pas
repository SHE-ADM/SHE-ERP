...
begin
  ...
  try
    ...
    // =========================================================================
    // LÓGICA DE RATEIO DO VOUTRO (Siscomex + AFRMM) COM AJUSTE DE CENTAVOS
    // =========================================================================
    LValSiscomex := 0;
    if Assigned(CEVTAXASISCOMEX) then 
    begin
    ...
    begin
      LValAfrmm := CEVAFRMM.Value;
      if LValAfrmm = 0 then LValAfrmm := FloatTextLocal(CEVAFRMM.Text);
    end;
    ...
    begin
      SomaOutrasDespesasRateio := 0; // Inicia o acumulador
      ...
      // Ajuste mágico: joga qualquer diferença (como 0.01 ou -0.01) no último item 
      // para que a soma detalhada (det>prod>vOutro) case EXATAMENTE com a soma total (total>vOutro)
      if High(ItensNFe) >= 0 then
        ItensNFe[High(ItensNFe)].vOutro := Round((ItensNFe[High(ItensNFe)].vOutro + (LTotalDespesas - SomaOutrasDespesasRateio)) * 100) / 100;
    end;
