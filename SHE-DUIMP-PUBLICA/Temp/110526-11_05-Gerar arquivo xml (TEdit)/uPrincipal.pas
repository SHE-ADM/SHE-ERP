...
begin
  ...
  try
    ...
    if Assigned(EditTaxaSiscomex) then
      EditTaxaSiscomex.Text := StringReplace(FormatFloat('0.00', ValorSiscomex), ',', '.', [rfReplaceAll]);
    ...
    begin
      ...
      ValorAFRMM := 0;
      if Assigned(EditvAFRMM) and (Trim(EditvAFRMM.Text) <> '') then
      begin
        T := Trim(EditvAFRMM.Text);
        T := StringReplace(T, '.', DecimalSeparator, [rfReplaceAll]);
        T := StringReplace(T, ',', DecimalSeparator, [rfReplaceAll]);
        ValorAFRMM := StrToFloatDef(T, 0);
      end;
...
var
  ...
  QtdItensICMSPendente: Integer;
  QtdItensVBCICMSPendente: Integer;
  QtdItensICMSFallbackPorDentro: Integer;
  ...
  begin Result := StrToIntDef(OnlyDigitsLocal(S), 0); end;
  ...
  begin
    Result := 0;
    S := JsonTextAnyLocal(AObjItem, ['caracterizacao.paisDeOrigem.codigo', 'caracterizacao.paisOrigem.codigoPais', 'origem.codigoPais', 'mercadoria.paisOrigem.codigo', 'paisOrigem.codigo']);
  ...
  begin
    Result := 0;
    S := JsonTextAnyLocal(ARoot, ['carga.paisProcedencia.codigo', 'carga.paisOrigem.codigo', 'paisProcedencia.codigo']);
    Result := CodigoPaisTTCEFromTextoLocal(S);
  end;
  ...
  function ContarItensICMSPendenteLocal: Integer;
  var K: Integer;
  begin Result := 0; for K := 0 to High(ItensNFe) do if ItemTemICMSPendenteLocal(ItensNFe[K]) then Inc(Result); end;
  ...
  begin
    ...
    for K := 0 to High(ItensNFe) do
      if ItensNFe[K].ICMS.modBC < 0 then begin ItensNFe[K].ICMS.modBC := ICMSModBCValor; Inc(QtdItensModBCERP); end;
  end;
  ...
  begin
    QtdItensICMSFallbackPorDentro := 0;
    for K := 0 to High(ItensNFe) do
    begin
      ...
      Inc(QtdItensICMSFallbackPorDentro);
    end;
  end;
  ...
  begin
    if AItem.ICMS.Orig = 0 then AItem.ICMS.Orig := 1;
    if Trim(AItem.ICMS.CST) = '' then AItem.ICMS.CST := '00';
    if Trim(AItem.PIS.CST) = '' then AItem.PIS.CST := '01';
    if Trim(AItem.COFINS.CST) = '' then AItem.COFINS.CST := '01';
  end;

  procedure AplicarCSTIPIPorValorLocal(var AItem: TDuimpNFeItemCompleto);
  begin
    if AItem.IPI.vIPI > 0 then AItem.IPI.CST := '00' else AItem.IPI.CST := '49';
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if CodigoPais <= 0 then CodigoPais := ExtrairCodigoPaisGeralLocal(RootGeral);
      ...
      AplicarCSTIPIPorValorLocal(ItensNFe[K]);
    end;
  end;
...
begin
  ...
  try
    ...
    ICMSModBCValor := -1; ICMSModBCInvalido := False; QtdItensModBCERP := 0;
    ...
    ValorAfrmmERPTexto := TextoEditAnyPorNome(['EditvAFRMM']);
    ...
    // =========================================================================
    LValSiscomex := FloatTextLocal(TextoEditAnyPorNome(['EditTaxaSiscomex']));
    LValAfrmm    := FloatTextLocal(TextoEditAnyPorNome(['EditvAFRMM']));
